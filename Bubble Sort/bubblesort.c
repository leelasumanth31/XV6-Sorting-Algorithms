...
void bubbleMonthSortArray(char** data, char** data2){
...
void bubbleSortArray(char** data){
   for(int i=0;i<=lines-1;i++){
      for(int j=i+1;j<=lines-1;j++){
         if(strcmp(data[i],data[j])>0){
            swap(data, i, j);
...
void bubbleSortNumberArray(char** data){
   for(int i=0;i<lines-1;i++){
      for(int j=i+1;j<lines-1;j++){
         if(atoi(data[i])>atoi(data[j])){
            swap(data, i, j);
         }
      }
   }
...
void bubbleBlankSortArray(char** data){
   for(int i=0;i<=lines-1;i++){
      for(int j=i+1;j<=lines-1;j++){
        if(strcmp(trimLeadingSpaces(data[i]),trimLeadingSpaces(data[j]))>0){
            swap(data, i, j);
         }
      }
   }
...
int main(int argc,char *argv[]) {
    ...
    if(argc >= 3){
        ...
        if(strcmp(argv[1],"-M")==0){
            ...
            getData(1,argc,argv,data);
            //getData(2, argv, data);
            bubbleMonthSortArray(data, data2);
            monthSortArray(data2);
            monthPrintData(data,data2,3);
        }
